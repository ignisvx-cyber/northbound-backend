...
async function callGemini({apiKey,model,payload}){
  ...
  for(let attempt=0;attempt<3;attempt++){
    ...
    if(attempt<2) await sleep(700*(2**attempt)+Math.floor(Math.random()*350));
  }
...
export async function geminiText({prompt,system='',model}){
  ...
  if(/high demand|unavailable|overload|temporar/i.test(raw)){
    throw new Error('A IA está temporariamente sobrecarregada. A NORTHBOUND tentou modelos alternativos; aguarde alguns segundos e envie novamente.');
  }
